...
def updateProfile():
    ...
    if request.json:
        ...
        if  userId[0] > raw_df.index[-1]:
            ...
            # return raw_df.loc[userId[0]].to_json()

        else:
            raw_df.at[userId[0],'Bios']= new_profile['Bios'][0]
            raw_df.at[userId[0],'Movies']= new_profile['Movies']
            raw_df.at[userId[0],'TV']= new_profile['TV']
            raw_df.at[userId[0],'Religion']= new_profile['Religion']
            raw_df.at[userId[0],'Music']= new_profile['Music']
            raw_df.at[userId[0],'Sports']= new_profile['Sports']
            raw_df.at[userId[0],'Books']= new_profile['Books']
            raw_df.at[userId[0],'Politics']= new_profile['Politics']

            raw_df.to_pickle('profiles.pkl')
            raw_df.to_csv('_new_profile.csv')
        ...
        top_10_sim = corr[[user_n]].sort_values(by=[user_n],axis=0, ascending=False)[1:50]
...
def savingNewProfile():
    ...
    if request.json:
        ...
        # Creating a DF with the Top 10 most similar profiles
        top_10_sim = corr[[user_n]].sort_values(by=[user_n],axis=0, ascending=False)[1:51]
